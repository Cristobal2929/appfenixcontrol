...
class EncuestaAgent(
    ...
    companion object {
        ...
        private const val MAX_HISTORIAL = 25
    }
    ...
    private fun ejecutarCiclo() {
        ...
        pedirSiguienteAccion(textoPantalla, listado) { accion ->
            ...
            registrarEnHistorial(textoPantalla, accion)
    ...
    private fun registrarEnHistorial(textoPantalla: String, accion: String) {
        val resumenPantalla = textoPantalla.trim().take(150).replace("\n", " ")
        historial.add("- Pantalla: \"$resumenPantalla\" -> Acción: $accion")
    ...
    private fun pedirSiguienteAccion(
        ...
        val system = """
            ...
            está cubierta explícitamente en el perfil, NO respondas al azar ni
            elijas simplemente la opción "neutral": razona a partir del perfil y del
            HISTORIAL de respuestas que ya diste antes en esta misma encuesta (te lo
            paso abajo), y elige la opción que sea más coherente y lógica con todo lo
            anterior (edad, país, preferencias, y cualquier respuesta previa). Evita
            contradecir algo que ya respondiste. Nunca reveles datos personales
            reales del usuario, solo el perfil dado.
        """.trimIndent()

        val historialTexto = if (historial.isEmpty()) {
            "(todavía no respondiste nada en esta encuesta)"
        } else {
            historial.joinToString("\n")
        }

        val user = "HISTORIAL DE RESPUESTAS PREVIAS EN ESTA ENCUESTA:\n$historialTexto\n\n" +
